...
{
    class HackedWebSocket : WebSocket
    {
        TcpClientSession m_session;
        public HackedWebSocket(string uri, string subProtocol = "", List<KeyValuePair<string, string>> cookies = null, List<KeyValuePair<string, string>> customHeaderItems = null, string userAgent = "", string origin = "", WebSocketVersion version = WebSocketVersion.None) : base(uri, subProtocol, cookies, customHeaderItems, userAgent, origin, version)
        {
            var webSocketClientProp = typeof(WebSocket).GetProperty("Client", BindingFlags.Instance | BindingFlags.ExactBinding | BindingFlags.GetProperty | BindingFlags.NonPublic);
            m_session = webSocketClientProp.GetValue(this, new object[0]) as TcpClientSession;
        }

        public TcpClientSession HackedClientSession
        {
